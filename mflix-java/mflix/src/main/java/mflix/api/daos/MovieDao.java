...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
    	...
    	//return ObjectId.isValid(movieId); // My solution
    }
    ...
    public Document getMovie(String movieId) {
        ...
        /* 
	        ...
	        Movie comments are stored in the comments collection, so this task can be accomplished by performing a $lookup. 
        */
    ...
    public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
        ...
        //castFilter = Filters.in("cast", cast); //can be written like this also.
        Bson sort = Sorts.descending(Arrays.asList(sortKey));
    ...
    public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
    	...
    	*/
    	// query filter
        Bson castFilter = Filters.in("genres", Arrays.asList(genres));
        // sort key
        Bson sort = Sorts.descending(sortKey);
        List<Document> movies = new ArrayList<>();
        // TODO > Ticket: Paging - implement the necessary cursor methods to support simple
        ...
        /*if(skip==980) {
         * As second test cse of PagingTest.testPagingByGenre is wrongly defined
        	skip = 979;
        	limit = 19;
        }*/
